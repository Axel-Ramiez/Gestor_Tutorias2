 ...
 * Descripción: Controlador para el formulario de registro y edición de Usuarios (Administrador).
 */
public class FXMLUsuario implements Initializable {
    ...
    @FXML private Label lbErrorContrasena;
    ...
    private void clicGuardar(ActionEvent event) {
        ...
        try {
            if (UsuarioDAO.esNoPersonalRegistrado(tfNoPersonal.getText(), idExcluir)) {
                lbErrorNoPersonal.setText("Este No. Personal ya está registrado.");
    ...
    private boolean validarCampos() {
        ...
        if (!Validacion.validarLongitud(tfNoPersonal, lbErrorNoPersonal, 1, 20)) valido = false;
        else if (!Validacion.validarNoPersonal(tfNoPersonal, lbErrorNoPersonal)) valido = false;

        if (!Validacion.validarLongitud(tfNombre, lbErrorNombre, 1, 150)) valido = false;
        else if (!Validacion.validarNombre(tfNombre, lbErrorNombre)) valido = false;
        ...
        if (!Validacion.validarLongitud(pfContrasena, lbErrorContrasena, 5, 50)) valido = false;

        if (!Validacion.validarSeleccion(cbRol, lbErrorRol, "Selecciona un rol")) valido = false;
