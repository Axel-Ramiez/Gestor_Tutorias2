 ...
 * Descripción: Controlador para la creación de nuevos reportes de tutoría.
 * Gestiona el formulario de registro y actualiza el estado de riesgo del estudiante.
 */
public class FXMLReporteTutoriaCrear {
    ...
    private void initialize() {
        ...
        cbPeriodo.valueProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue != null) {
                ...
                if (cbFechaReporte.getItems() != null) {
    ...
    public void clicGuardar(ActionEvent event) {
        ...
        try {
            ...
            if (idGenerado > 0) {
                boolean enRiesgo = !chkAsistencia.isSelected();
    ...
    private <T> int obtenerIdSeleccionado(ComboBox<T> combo, Function<T, Integer> extractorId) {
